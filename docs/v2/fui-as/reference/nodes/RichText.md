...
- **Overarching styling:** `font(...)`, `fontStack(...)`, `fontFamily(...)`, `fontWeight(...)`, `fontStyle(...)`, `fontSize(...)`, `textColor(...)`
...
`RichText` compiles helper-span fragments into one attributed retained `Text` node, so shaping stays unified across span boundaries. Container-level font and color setters act as defaults for spans that do not override them.
