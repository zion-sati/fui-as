# FUI-AS SDK Docs Index (v2)

This is the primary SDK navigation page for `v2/fui-as`.

## Accessibility

- [Accessibility and semantics](./ACCESSIBILITY_AND_SEMANTICS.md)
- [Text input behavior guide](./TEXT_INPUT_DESIGN.md)
- [Events and callbacks](./EVENTS_AND_CALLBACKS.md)

## Controls

- [Controls and nodes overview](./CONTROLS_AND_NODES.md#controls)
- [Per-type control reference](./reference/README.md#controls)
- [Keyboard policy](./KEYBOARD_POLICY.md)
- [Overlays and portals](./OVERLAYS_AND_PORTALS.md)

## Nodes

- [Controls and nodes overview](./CONTROLS_AND_NODES.md#nodes)
- [Core layout concept (`width(100%)` vs `fillWidth()`)](./CONTROLS_AND_NODES.md#core-layout-concept)
- [Layout sizing guide (`fill*` vs `Unit.Percent`)](./CONTROLS_AND_NODES.md#layout-sizing-guide-fill-vs-unitpercent)
- [Per-type node reference](./reference/README.md#nodes)
- [Events and callbacks](./EVENTS_AND_CALLBACKS.md#node-level-pointer-callbacks)

## Core

- [API reference](./API_REFERENCE.md)
- [Browser file bridge reference](./API_REFERENCE.md#browser-file-bridge)
- [Browser fetch bridge reference](./API_REFERENCE.md#browser-fetch-bridge)
- [Accessibility and semantics](./ACCESSIBILITY_AND_SEMANTICS.md)
- [Events and callbacks](./EVENTS_AND_CALLBACKS.md)
- [Controls and nodes overview](./CONTROLS_AND_NODES.md#common-node-state-apis)
- [Browser file bridge quickstart](./QUICKSTART.md#6-browser-file-bridge)
- [Browser fetch bridge quickstart](./QUICKSTART.md#7-browser-fetch-bridge)
- [JS host services quickstart](./QUICKSTART.md#8-js-host-services-and-host-events)
- [Text input reference](./TEXT_INPUT_REFERENCE.md)

## Theme

- [Theming and style matrix](./THEMING_STYLE_MATRIX.md)
- [Controls and nodes](./CONTROLS_AND_NODES.md)
- [Text input behavior guide](./TEXT_INPUT_DESIGN.md)

## Navigation

- [FUI-AS quickstart](./QUICKSTART.md)
- [Per-type reference](./reference/README.md)
- [Top-level v2 quickstart](../../QUICKSTART.md)

## See also

- [FUI-AS API reference](./API_REFERENCE.md)
- [FUI-AS per-type reference](./reference/README.md)
- [FUI-AS accessibility and semantics](./ACCESSIBILITY_AND_SEMANTICS.md)
