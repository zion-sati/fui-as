...
async function successfulFuiAsCiRuns() {
  const runs = [];
  for (let page = 1; page <= 10; page += 1) {
    const result = await api(`/repos/${repository}/actions/workflows/fui-as-ci.yml/runs?event=push&status=success&per_page=100&page=${page}`);
