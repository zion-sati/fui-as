...
function writeTextPartsPayload(session: HarnessAppSession, values: readonly string[], context: string): number {
  ...
  for (let index = 0; index < encodedValues.length; index += 1) {
    byteOffset = writeLengthPrefixedText(session.memory, session.textBufferPtr, byteOffset, encodedValues[index] ?? new Uint8Array());
  }
  return totalBytes;
}
    ...
    imports: {
      fui_fetch_start(
        ...
        void fetch(url, {
          method,
          headers,
          body: bodyBytes.length > 0 ? copyBytesToArrayBuffer(bodyBytes) : undefined,
